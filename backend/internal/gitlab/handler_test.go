...
func TestRoutesGetAnalysisJob(t *testing.T) {
	...
	validator, err := authn.NewValidator("gitlab-handler-test-secret")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoutesGetRepositoryIncludesLatestAnalysisJobID(t *testing.T) {
	...
	if payload.Data.ID != service.repo.ID || payload.Data.LatestAnalysisJobID != latestAnalysisJobID {
		t.Fatalf("unexpected data: %+v", payload.Data)
	}
}
...
func issueToken(t *testing.T, userID string) string {
	t.Helper()
	tm, err := auth.NewTokenManager("gitlab-handler-test-secret", time.Hour)
	if err != nil {
		t.Fatal(err)
	}
