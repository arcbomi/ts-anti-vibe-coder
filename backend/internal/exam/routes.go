...
func NewRouter(handler *Handler, validator *authn.Validator) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		r.Post("/internal/preparations", handler.PrepareSucceededProject)
	})
	r.Group(func(r chi.Router) {
		r.Use(middleware.RequireJWTIdentity(validator))
		r.Get("/succeeded-projects", handler.ListSucceededProjects)
		r.Post("/succeeded-projects/{slug}/prepare", handler.StartSucceededProjectPreparation)
