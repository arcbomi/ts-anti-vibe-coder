...
import { buildApiGateway } from "../src/app.js";

const TEST_SECRET = "gateway-test-secret";
const TEST_USER_ID = "00000000-0000-0000-0000-000000000001";

test("api gateway routes requests to the expected upstream service", async () => {
  const seenRequests: Array<{ url: string; userId: string | null }> = [];
  const app = await buildApiGateway({
    env: testEnv(),
...
test("api gateway rejects protected routes without a bearer token", async () => {
  const app = await buildApiGateway({
    env: testEnv(),
...
test("api gateway rejects expired tokens", async () => {
  const app = await buildApiGateway({
    env: testEnv(),
...
test("api gateway returns 404 for unknown routes", async () => {
  const app = await buildApiGateway({
    env: testEnv(),
    fetchImpl: async () => new Response("ok", { status: 200 })
  });
