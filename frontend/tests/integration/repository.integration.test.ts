...
afterEach(async () => {
  ...
  const { repositoryStore } = await import('../../src/domains/repository/store/repositoryStore')
...
describe('repository integration flow', () => {
  it('syncs Tomorrow projects, lets the user choose a repo, checks bot access, and enables question work', async () => {
    const { default: RepositoryConnectSection } = await import('../../src/domains/repository/pageSection/RepositoryConnectSection.vue')
    const { default: RepositoryStatusSection } = await import('../../src/domains/repository/pageSection/RepositoryStatusSection.vue')
    ...
    await renderRouteView({
      initialEntry: '/repository/connect',
      routes: [
        { path: '/repository/connect', component: RepositoryConnectSection },
        { path: '/repository/status', component: RepositoryStatusSection },
      ],
    })
    ...
    await click(screen.getByRole('button', { name: /refresh from tomorrow/i }))
    expect(await screen.findByText(/synced 1 succeeded project/i)).toBeTruthy()
    await click(screen.getByRole('button', { name: /choose this repo/i }))
    ...
    await click(screen.getByRole('button', { name: /i already added the bot/i }))
  ...
  it('shows a friendly access denied message when the bot is not a collaborator', async () => {
    ...
    await click(await screen.findByRole('button', { name: /refresh from tomorrow/i }))
    await click(await screen.findByRole('button', { name: /choose this repo/i }))
    await click(await screen.findByRole('button', { name: /i already added the bot/i }))
  ...
  it('shows a clear sync error when Tomorrow refresh fails', async () => {
    ...
    await renderRouteView({
      initialEntry: '/repository/connect',
      routes: [{ path: '/repository/connect', component: RepositoryConnectSection }],
    })
